...
int vid_print_char_alpha (lcd_info *p,int horiz_offset, int vert_offset, int color, char character, int background_color, struct abc_font_struct font[], alt_video_display * display)
{
  ...
  for(i = 0; i < font[character-33].bounds_height; i++) {
    for (j = 0; j < font[character-33].bounds_width; j++) {
      ...
      vid_set_pixel(p,(horiz_offset + j), (vert_offset + i), new_color, display);
...
int vid_print_string_alpha(lcd_info *p,int horiz_offset, int vert_offset, int color, int background_color, struct abc_font_struct font[], alt_video_display * display, char string[])
{
  int i = 0;
  int tab;
  ...
  while (string[i]) {
    ...
    if(string[i] == 32)  // this is a space in ASCII
    {
      ...
      {
        vid_print_char_alpha (p,horiz_offset, vert_offset, background_color, '-', background_color, font, display);
      }
    ...
    {
      vid_print_char_alpha (p,horiz_offset, vert_offset, color, string[i], background_color, font, display);
