...
void breakout_init_textures(game_struct * game)
{
	...
	display_add_text(display, 175, 210, 0xffffff, tahomabold_32, "Loading textures...");
	...
	pic32_sendrpc(game->pic32_handle, "textures.bin", 13, CYCLONE_RPC_FILE);
	while(!pic32_receive(game->pic32_handle, (char **) &textures, &len, 0));
	...
	display_add_text(display, 500, 210, 0xffffff, tahomabold_32, "OK");
	display_end_frame(display);
...
void breakout_uninit(game_struct * g)
{
	...
	display_remove_sprite(g->display_handle, g->ball.s);
	display_remove_sprite(g->display_handle, g->paddle);
...
void breakout_ball_paddle(ball * ball, sprite paddle)
{
	...
	double ospeedx = ball->v.x, speedx = ball->v.x;
	double ospeedy = ball->v.y , speedy = ball->v.y;
...
{
	display_info * display = g->display_handle;
	...
	display_clear_screen(display);
	display_add_text(display, 250, 210, 0xffffff, tahomabold_32, "Loading game...");
	display_end_frame(display);
	...
	for(i=0; i< 12; i++) {
		for(j=0; j < 14; level++) {
			...
			if(g->bricks[i*14+j].value > 0 && g->bricks[i*14+j].value < 5) {
				display_sprite_init(display, &g->bricks[i*14+j].s, 45+j*50+j, 45+i*20+i, 50, 20, display->bricks_img[g->bricks[i*14+j].value-1], 0, 0);
				g->rbricks++;
				j++;
			}
			else if(*level == '*') {
	...
	// Initialize paddle
	display_sprite_init(display, &g->paddle, 300,440, 200, 20, (alt_u32*) TEXTURES_BASE + IMG_PADDLE200, (alt_u8*) TEXTURES_BASE + ALPHA_PADDLE200*4, 1);

	// Initialize ball
	display_sprite_init(display, &g->ball.s, g->paddle.x+100,g->paddle.y-20, 20,20, (alt_u32*) TEXTURES_BASE + IMG_BALL, (alt_u8*) TEXTURES_BASE + ALPHA_BALL*4, 2);
	g->ball.v.x = sqrt(2)/2;
	g->ball.v.y = -sqrt(2)/2;

	// Initialize walls
	display_sprite_init(display, &g->walls[0], 0,0, 10, 440, display->wall_vert_img, 0, 0);
	display_sprite_init(display, &g->walls[1], 0,0, 800, 10, display->wall_horiz_img, 0, 0);
	display_sprite_init(display, &g->walls[2], 790,0, 10, 440, display->wall_vert_img, 0, 0);
	...
	// Display background
	sprite s ;
	display_sprite_init(display, &s, 0,0, 800, 480, (alt_u32*) display->frame_buffer[0], 0, 0);
		...
		case ADD_BRICK:
			...
			while(!brick_placed)
			{
				...
				if(!g->bricks[rnd].enabled && !breakout_ball_collision(g->ball, g->bricks[rnd].s, &(g->ball.s.x), &(g->ball.s.y)))
				{
					...
					display_sprite_init(display, &g->bricks[rnd].s,  45+(rnd%14)*50+(rnd%14), 45+(rnd/14)*20+(rnd/14), 50, 20, display->bricks_img[g->bricks[rnd].value-1], 0, 0);
					g->rbricks++;
